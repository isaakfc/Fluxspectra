...
//            g.fillRoundedRectangle(5.f, bounds.removeFromBottom(scaledY));
        bounds = bounds.removeFromLeft(scaledX);
        g.fillRoundedRectangle(bounds, 0.f);
        
        if (level > 0)
        {
            float temp = level - 60.f;
            const auto scaledXRight = juce::jmap(temp, -60.f,6.f,0.f,static_cast<float>(getWidth()));
            bounds = bounds.removeFromRight(scaledXRight);
            g.setColour(MyColours::blue);
            g.fillRoundedRectangle(bounds, 0.f);
        }
